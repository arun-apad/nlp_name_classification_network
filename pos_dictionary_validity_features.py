...
def get_features(df,newdf):
    for i, row in df.iterrows():
        ...
        try:
            ...
            newdf.set_value(i,'word_count',0)
            pass
        #if(i>5):
            #break
...
def get_all_features():
    ...
    newdf = pd.DataFrame()
    newdf = get_features(df,newdf)    
    #print(newdf)
    
    newdf['last_name_low_tag'] = newdf['last_name_low_tag'] == 0
    
    filename = 'datasets/pos_dictionary_validity.csv'
    newdf.to_csv(filename, index=False, encoding='utf-8')
    
    
    print("Data is porcessed to model features of a person's name, engineered based on Parts of speech, dictionary synonyms and validity.")
